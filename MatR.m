...
% For windows we need to change the path seperators
...
classdef MatR < handle
   ...
   methods
      function self = MatR()
         ...
         path2 = [ path filesep 'utils.R'];
         s=regexp(path2,filesep,'split');
         Rpath=s{1};
         for i=2:size(s,2)
         Rpath=[Rpath '/' s{i}];
         end
      ...
      function assign(self,name,value)  % Here I modify the assign to be able to assign matrix
         import org.rosuda.REngine.*;
         [row,col]=size(value);
         if(min(row,col)==1)
         self.connection.assign(MatR.jstr(name),value);
         else
         self.connection.assign(MatR.jstr(name),REXP.createDoubleMatrix(value));    
         end
      end
